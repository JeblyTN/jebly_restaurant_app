...
class OrderDetailsController extends GetxController {
  ...
  Future<void> getArgument() async {
    ...
    if (argumentData != null) {
      ...
      //             amount: (subTotal.value - double.parse(orderModel.value.discount.toString()) - specialDiscount.value).toString(),
      //             taxModel: element);
      //   }
      // }
      //
      ...
      if (result == true) {
        showDialog(
          ...
          builder: (_) {
            ...
            final backgroundColor = isDarkMode ? AppThemeData.grey900 : AppThemeData.grey50;
            final textColor = isDarkMode ? AppThemeData.surface : AppThemeData.surfaceDark;
  ...
  Future<dynamic> showLoadingAlert() {
    return showDialog(
      context: Get.context!,
                  ...
                  : Column(mainAxisSize: MainAxisSize.min, children: [
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          BluetoothInfo device = availableBluetoothDevices[index];
  ...
  Future<List<int>> getTicket() async {
    ...
    bytes += generator.text(orderModel.value.vendor!.title.toString(), styles: const PosStyles(align: PosAlign.center, height: PosTextSize.size2, width: PosTextSize.size2, bold: true), linesAfter: 1);
    ...
    bytes += generator.text('Date: ${DateFormat('MMM d yyyy, h:mm:ss a').format(DateTime.fromMicrosecondsSinceEpoch(orderModel.value.vendor!.createdAt!.microsecondsSinceEpoch)).toString()} ',
        ...
        PosColumn(
            text: Constant.amountShow(
                amount: Constant.calculateTax(
                        amount: (double.parse(subTotal.value.toString()) - double.parse(orderModel.value.discount.toString()) - specialDiscountAmount.value).toString(), taxModel: taxModel)
                    .toString()),
            width: 5,
            styles: const PosStyles(
              align: PosAlign.right,
    ...
    bytes += generator.row([
      ...
      PosColumn(
          text: Constant.amountShow(amount: totalAmount.value.toString()), width: 5, styles: const PosStyles(align: PosAlign.right, height: PosTextSize.size1, width: PosTextSize.size1, bold: true)),
      PosColumn(
        text: '',
        width: 1, // Spacer column
      ),
